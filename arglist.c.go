...
func newArgList(ctx *Environment, params ...interface{})(alp ArgList, err os.Error){
	//fmt.Printf("newArgList: %+v\n", params...)
	alp = make(ArgList,0)
	for i, param := range(params){
		var ok C.int
		switch v := param.(type) {
			case int: 	alp = append(alp, C.intValue(C.jint(v)))
			case int64: 	alp = append(alp, C.longValue(C.jlong(v)))
			case C.jstring: alp = append(alp, C.objValue(v))
			case C.jboolean: alp = append(alp, C.boolValue(v))
			case C.jint: 	alp = append(alp, C.intValue(v))
			case C.jobject: alp = append(alp, C.objValue(v))
			case *Object: alp = append(alp, C.objValue(v.object))
			case *Class: alp = append(alp, C.objValue(v.class))
			case C.jvalue: alp = append(alp, v)
			case string:
				var str *Object
			...
			default: err = os.NewError(fmt.Sprintf("Unknown type: %T", v))
		}
		if ok != 0 { err = os.NewError("Couldn't parse arg #" + strconv.Itoa(i+1))}
		if err != nil { break }
	}
	return
}
